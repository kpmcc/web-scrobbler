...
// On page load, episode-player is present, but no child player
// after clicking play,  episode-player.parent class list is updated to contain
// episode player type ['is-soundcloud', mixcloud?]
//
// Trying to load old mixcloud players in firefox isn't working because of
// third party cookie blocking

// Similar to how the bandcamp connector splits up by page type,
// we will probably want to split behavior based on whether player is mixcloud
// or soundcloud

function actualSetup() {
	Util.debugLog('calling actualSetup');
	Connector.playerSelector = '.soundcloud-player';
	Connector.trackSelector = '.track__title';
	Connector.artistSelector = '.track__artist';
	Connector.progressBarSelector =
		'.soundcloud-player__progress-bar-container';

	Connector.isPlaying = () => {
		const progressBar = document.querySelector(this.progressBarSelector);
		if (progressBar) {
			const pbClassList = progressBar.classList;
			if (pbClassList) {
				return !pbClassList.contains('visually-hidden');
			}
		}
	};

	if (playerIsSoundcloud()) {
		Util.debugLog('player is soundcloud');
	}
}
...
function setupConnector() {
	Util.debugLog('calling setupConnector');
	Connector.playerSelector = '.episode-player';
	Connector.trackSelector = '.track__title';
	Connector.artistSelector = '.track__artist';
	Connector.progressBarSelector =
		'.soundcloud-player__progress-bar-container';
	Util.debugLog('end of setupConnector');
	//if (playerIsSoundcloud()) {
	//	Util.debugLog('player is soundcloud');
	//}
	// initEventListeners();
	...
	Connector.getUniqueID = () => {
		if (document.querySelector('.bcweekly.playing') !== null) {
			...
			if (currentShowId in bandcampWeeklyData) {
				...
				const currentTrackIndex = Util.getAttrFromSelectors(
					'.bcweekly-current',
					'data-index'
				);
...
function playerIsSoundcloud() {
	return document
		.querySelector('.episode-player')
		.parentElement.classList.contains('is-soundcloud');
}
